...
export default function AddReviewGame({ navigation, route }) {
    ...
    const addReview  = async() => {
        ...
        const game = responseGetGame.document
        const ratingTotal = game.ratingTotal + rating
        const quantityVoting = game.quantityVoting + 1
        const ratingResult = ratingTotal / quantityVoting
        const responseUpdateGame = await updateDocument("games", idGame, {
            ratingTotal,
            quantityVoting,
            rating: ratingResult
        })
        ...
        if (!responseUpdateGame.statusResponse){
            toastRef.current.show(
                "Error al actualizar el juego, por favor intentarlo mas tarde."
                , 3000
            )
            return
        }
    ...
    const validForm = () => {
        ...
        if (!rating){
            toastRef.current.show("Debes darle una puntuación al juego.", 3000)
            isValid = false
        }
